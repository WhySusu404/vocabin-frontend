...
class AdminAuthService {
    ...
    isAuthenticated() {
        ...
        console.log('🔐 Memory state - isAdminAuthenticated:', this.isAdminAuthenticated, 'adminUser:', this.adminUser);
        ...
        try {
            ...
            if (session) {
                ...
                console.log('🔐 Session age:', sessionAge, 'ms, max age:', maxAge, 'ms');
                console.log('🔐 Session user role:', sessionData.user?.role);
                
                if (sessionAge < maxAge && sessionData.user.role === 'admin') {
    ...
    async getDashboardStats() {
        try {
            // Mock admin stats for now - replace with real API calls later
            return {
                totalUsers: 1247,
                activeUsers: 892,
                totalWordLists: 156,
    ...
    async getUsers(filters = {}) {
        try {
            // Mock user data for now - replace with real API calls later
            const mockUsers = [
                {
                    id: '1',
